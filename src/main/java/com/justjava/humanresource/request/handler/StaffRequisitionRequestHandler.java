...
@Component @RequiredArgsConstructor public class StaffRequisitionRequestHandler implements WorkflowRequestTypeHandler {
 ...
 public void saveDetails(WorkflowRequest r,CreateWorkflowRequestCommand c){var p=c.getStaffRequisition(); StaffRequisitionDetail d=new StaffRequisitionDetail(); d.setWorkflowRequestId(r.getId()); applyPayload(d,p); repository.save(d);}
 public void updateDetails(WorkflowRequest r,CreateWorkflowRequestCommand c){var p=c.getStaffRequisition(); StaffRequisitionDetail d=repository.findByWorkflowRequestId(r.getId()).orElseGet(StaffRequisitionDetail::new); if(d.getWorkflowRequestId()==null){d.setWorkflowRequestId(r.getId());} applyPayload(d,p); repository.save(d);}
 private void applyPayload(StaffRequisitionDetail d, CreateWorkflowRequestCommand.StaffRequisitionPayload p){ d.setJobTitle(p.getJobTitle()); d.setDepartmentId(p.getDepartmentId()); d.setJobGradeId(p.getJobGradeId()); d.setNumberOfPositions(p.getNumberOfPositions()); d.setEmploymentType(p.getEmploymentType()); d.setRequisitionReason(p.getRequisitionReason()); d.setTargetStartDate(p.getTargetStartDate()); d.setBudgeted(p.isBudgeted()); d.setEstimatedMonthlyCost(p.getEstimatedMonthlyCost()); d.setReasonForHire(p.getReasonForHire()); d.setReplacementEmployeeId(p.getReplacementEmployeeId()); }
}